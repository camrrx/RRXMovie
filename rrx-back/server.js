...
app.get("/movies", async (req, res) => {
	return new Promise((resolve, reject) => {
		db.query(
			"SELECT * FROM movies WHERE users_idusers = ?",
			[req.id],
			(err, result) => {
...
app.post("/movies", async (req, res) => {
	const movie = {
		...
		rate: req.body.rateMovie,
		genre: req.body.genreMovie,
		users_id: req.body.userId,
		picture: req.body.pictureMovie,
	};
	return new Promise((resolve, reject) => {
		db.query(
			"INSERT INTO movies (id_movie, name, rate, genre, users_idusers, picture) VALUE (?,?,?,?,?,?)",
			[
				movie.id_movie,
				movie.name,
				movie.rate,
...
app.post("/movies/update", async (req, res) => {
	...
	return new Promise((resolve, reject) => {
		db.query(
			"UPDATE movies SET rate = ? WHERE users_idusers = ? AND id_movie=?",
			[movie.rate, movie.users_id, movie.id_movie],
			(err, result) => {
				if (err) {
					console.log(err);
					return reject(err);
	...
	return new Promise((resolve, reject) => {
		db.query(
			...
			(err, result) => {
				if (err || result.length) {
					console.log(
						"Error while tryin to register : the username already exists"
					);
					return reject(false);
				} else {
					return resolve(true);
	...
	return new Promise((resolve, reject) => {
		db.query(
			...
			(err, result) => {
				if (err || result.length) {
					console.log(
						"Error while tryin to register : the email already exists"
					);
					return reject(false);
				} else {
					return resolve(true);
...
app.get("/rate/:idUser/:idMovie", async (req, res) => {
	...
	db.query(
		"SELECT rate FROM movies WHERE users_idusers = ? AND id_movie = ? ",
		[idUser, idMovie],
		(err, result) => {
			if (err) {
				console.log("Error");
